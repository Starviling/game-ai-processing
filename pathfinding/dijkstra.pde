class DijkstraTag<T> extends Tag<T> {
  DijkstraTag(Node<T> node, float c, Edge<T> edge) {
    super(node, c, edge);
  }
}

class DijkstraPath<T> implements GraphSearch<T> {
  Node<T> goal;
  int iterations = 0;
  ...
  void reset(Node<T> start, Node<T> goal) {
    ...
    Tag<T> tag = new DijkstraTag<T>(start, 0, null);
    tags.put(start, tag);
    open.add(tag);
  }
  ...
  void update() {
    if(!tags.containsKey(goal)) {
       ...
       if(null != tag && !closed.contains(tag.node)) {
          if (!tag.node.occupied) {
             for(Edge<T> edge : tag.node.outgoing) {
               if(!tags.containsKey(edge.end)) {
                 // The cost from start  stored in tag
                 Tag<T> next_tag = new DijkstraTag<T>(edge.end, tag.c + edge.weight, edge);
